...
def inference_worker(model, feature_queue, result_queue, device):
    ...
    with torch.no_grad():
        while True:
            ...
            # L2 归一化（与训练/测试阶段保持一致）
            input_feature = torch.nn.functional.normalize(input_feature, p=2, dim=-1)
...
def print_session_summary(score_history, threshold, total_frames, elapsed_sec):
    ...
    # 整体异常判定
    if anomaly_ratio >= 30:
        verdict = "[高风险] 视频中存在大量异常行为，建议人工复查！"
...
def main():
    ...
    nhead             = 4      # 多头注意力头数
    chunk_size        = 16     # 流式处理片段长度（与 CLIP 采样帧数一致）
    ...
    try:
        ...
        print("[INFO] Check weight path and ensure xl_best.pth is in weights/")
        sys.exit(1)
    ...
    while video_stream.running:
        ...
        if not result_queue.empty():
            ...
            if current_score > anomaly_threshold:
                print(f"  Anomaly detected! Score: {current_score:.4f}  "
        ...
        elif current_score >= anomaly_threshold * 0.75:
        ...
        # 评分进度条
        draw_score_bar(frame, current_score, anomaly_threshold,
...
def print_banner():
    print("=" * 60)
    print("     StreamVAD-XL Deployment Script v1.0")
    print("=" * 60)
    print()
...
def check_python_version():
    ...
    if current < PYTHON_REQUIRED or current > PYTHON_MAX:
        ...
        print("[INFO] Create environment with: conda create -n streamvad_xl python=3.9")
        sys.exit(1)
...
def create_project_directory():
    print("[3/7] Creating project directory...")
    project_path = Path(PROJECT_NAME)
    if project_path.exists():
        print(f"[INFO] {PROJECT_NAME} already exists, skipping creation")
    else:
        try:
            project_path.mkdir()
            print(f"[SUCCESS] Created project directory: {PROJECT_NAME}")
        except Exception as e:
...
def print_post_install():
    ...
    print("Next steps:")
    print()
    print("1. Place XL model code:")
    print("   Copy model.py (containing StreamVADWithXL) and dataset.py")
    print("   into the StreamVAD_XL/ directory")
    print()
    print("2. Download XL model weights:")
    print("   Place xl_best.pth in StreamVAD_XL/weights/")
    print()
    print("3. Run realtime detection:")
    print("   python realtime_inference.py")
    print()
    print("4. Press 'q' to exit -- session summary will be printed")
    print()
    print("XL Model Parameters (defaults):")
    print("   feature_dim       = 512  (CLIP ViT-B/32 output)")
    print("   hidden_dim        = 256")
    print("   mem_len           = 32   (memory cache length)")
    print("   num_layers        = 2    (Transformer-XL layers)")
    print("   nhead             = 4    (attention heads)")
    print("   chunk_size        = 16   (streaming chunk length)")
    print("   anomaly_threshold = 0.5")
    print("=" * 60)
