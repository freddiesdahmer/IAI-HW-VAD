# StreamVAD-XL 
StreamVAD-XL 是基于 StreamVAD 的改进版本，通过引入 Transformer-XL 长期记忆机制，替代了原始的固定窗口注意力，实现了跨片段的时序状态传递，从而解决了原始框架缺失长距离时序上下文的问题。本指南将详细介绍 StreamVAD-XL 的快速部署、参数配置及推理运行流程。
---
## 与原版 StreamVAD 的核心区别
| 项目 | StreamVAD（原版） | StreamVAD-XL |
|------|-------------------|--------------|
| 模型类 | `LSHMA` | `StreamVADWithXL` |
| 时序建模 | 固定窗口注意力 | Transformer-XL 记忆缓存 |
| 特征处理 | 直接输入 | L2 归一化后输入 |
